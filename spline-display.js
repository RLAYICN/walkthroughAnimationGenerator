SplineDisplay = function(params)
{
  ...
  var ptToPix = function(pt)  { 
    return [ (self.width/2+(pt[0]*(self.width/self.xlen)))+0.5
      ,(self.height/2-(pt[1]*(self.height/self.ylen)))+0.5 
    ]; 
  }
  var pixToPt = function(pix) { 
    return [ (pix[0]-self.width/2)/(self.width/self.xlen)
      ,-(pix[1]-self.height/2)/(self.height/self.ylen)
    ]; 
}
  ...
  if(params.hasOwnProperty('spline'))
       self.spline     = params.spline;     
       else self.spline     = new Spline([[-1,1],[-1,-1],[1,-1],[1,1]],0);
  ...
  {
    if(!self.width)  
    self.width  = self.parentContainer.offsetWidth; 
    if(!self.width)  
    self.width  = self.parentContainer.width;
    if(!self.height) 
    self.height = self.parentContainer.offsetHeight; 
    if(!self.height) 
    self.height = self.parentContainer.height;
  ...
  var draw = function()
  {
        ...
        for(var k = 0; k < self.renderSplines[i].derivedPts[0][j].length; k++)
        {
          ...
          if(pass < self.drawldepth)
          {
            if(k < self.renderSplines[i].derivedPts[0][j].length-1)
              drawLine(self.renderSplines[i].derivedPts[0][j][k][0]
                ,self.renderSplines[i].derivedPts[0][j][k][1]
                ,self.renderSplines[i].derivedPts[0][j][k+1][0]
                ,self.renderSplines[i].derivedPts[0][j][k+1][1],
                skeletonCanvas);
          }
        }
        pass++;
    ...
    //console.log(lastDrawnPt.length);
    console.log(lastCalculatedPt);
