var Spline = function(pts, chainlen){ 
  var self = this;
  self.pts = pts;
  self.chainlen = (chainlen === undefined || chainlen == 0) ? self.pts.length : chainlen; 
...
self.ptForT = function(t){
    if(t == self.t) 
        return self.calculatedPt; //no need to recalculate
    t = (t+100)%1;
    self.t = t;
    ...
    while(pass < self.chainlen){
        for(var i = 0; i < self.derivedPts[chain][pass-1].length-1; i++)
            self.interpAPt(self.derivedPts[chain][pass-1][i]
              ,self.derivedPts[chain][pass-1][i+1],t
              ,self.derivedPts[chain][pass][i]);
        pass++;
    }
